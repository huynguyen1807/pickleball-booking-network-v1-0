...
const BOOKING_STATUS_MAP: Record<string, { label: string; bg: string; color: string }> = {
    ...
    completed: { label: 'Hoàn thành',   bg: 'rgba(59,130,246,0.15)',   color: '#3b82f6' },
...
export default function UserDashboard() {
    ...
    const visibleBookings = showAllBookings ? allBookings : allBookings.slice(0, PAGE_SIZE)
            ...
            <div style={{ display: 'flex', gap: 4, marginBottom: 16, background: 'var(--bg-glass)', borderRadius: 12, padding: 4, width: 'fit-content', border: '1px solid var(--border-glass)' }}>
                {(['bookings', 'payments'] as ActiveTab[]).map(tab => (
                    <button key={tab} onClick={() => setActiveTab(tab)} style={{
                        ...
                        {tab === 'bookings' ? `🏟️ Lịch đặt sân (${allBookings.length})` : `💰 Thanh toán (${visiblePaymentHistory.length})`}
                    </button>
                ))}
            </div>
                ...
                <div className="glass-card">
                    ...
                    {allBookings.length === 0 ? (
                        <div style={{ textAlign: 'center', padding: '48px 20px', color: 'var(--text-muted)' }}>
                            ...
                            <p style={{ fontWeight: 600, marginBottom: 8 }}>Chưa có booking nào</p>
                                ...
                                <table className={styles.table}>
                                    ...
                                    <tbody>
                                        {visibleBookings.map((b, i) => {
                                            const st = BOOKING_STATUS_MAP[b.status] || { label: b.status, bg: 'rgba(156,163,175,0.15)', color: '#9ca3af' }
                                            ...
                                            return (
                                                <tr key={b.id}>
                                                    ...
                                                    <td>
                                                        <span style={{ padding: '3px 10px', borderRadius: 999, fontSize: '0.75rem', fontWeight: 600, background: st.bg, color: st.color, whiteSpace: 'nowrap' }}>
                                                            {st.label}
                                                        </span>
                                                    </td>
                                                    <td>
                                                        {canCancel && (
                                                            <button
                                                                ...
                                                                style={{ background: 'rgba(239,68,68,0.12)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 6, padding: '3px 10px', cursor: 'pointer', color: '#ef4444', fontSize: '0.72rem', fontWeight: 600 }}>
                            ...
                            {allBookings.length > PAGE_SIZE && (
                                <div style={{ textAlign: 'center', marginTop: 16, paddingTop: 16, borderTop: '1px solid var(--border-glass)' }}>
                                    <button onClick={() => setShowAllBookings(v => !v)} style={{ background: 'none', border: '1px solid var(--border-glass)', borderRadius: 8, padding: '6px 20px', cursor: 'pointer', color: 'var(--text-secondary)', fontSize: '0.85rem', fontWeight: 600 }}>
                                        {showAllBookings ? '▲ Thu gọn' : `▼ Xem thêm ${allBookings.length - PAGE_SIZE} booking`}
                                    </button>
                                </div>
                            )}
                        </>
